...
// Schools routes
app.get("/schools", authMiddleware, roleMiddleware(["main_admin", "school_admin"]), async (req, res) => {
  try {
    ...
    try {
      const query =
        req.user.role === "school_admin"
          ? { _id: req.params.id, school_id: req.user.school_id }
          : req.user.role === "teacher"
          ? { _id: req.params.id, teacher_id: req.user.userId }
          : { _id: req.params.id };
...
app.get(
  ...
  authMiddleware,
  roleMiddleware(["school_admin", "main_admin", "parent", "student"]),
  async (req, res) => {
    try {
...
// Analytics route
app.get("/analytics", authMiddleware, roleMiddleware(["main_admin", "school_admin"]), async (req, res) => {
  try {
    const isSchoolAdmin = req.user.role === 'school_admin';
    const schoolFilter = isSchoolAdmin ? { school_id: req.user.school_id } : {};
    ...
    const usersByRole = {
      ...
      mainAdmins: isSchoolAdmin ? 0 : await User.countDocuments({ role: "main_admin" }),
    };

    const totalSchools = isSchoolAdmin ? 1 : await School.countDocuments();
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      if (password) updateData.password = await bcrypt.hash(password, 10); // Assuming bcrypt for password hashing
